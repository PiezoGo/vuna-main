...
export default function FarmerDashboard() {
  ...
  const [activeTab, setActiveTab] = useState('listings'); // listings, orders, chats, calculator
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      <div className="flex border-b border-gray-200 mb-6">
        {['listings', 'orders', 'chats', 'calculator'].map((tab) => (
          <button
            key={tab}
          ...
          >
            {tab === 'orders' ? `Orders (${orders.length})` : tab === 'chats' ? `Inbox (${chats.length})` : tab}
          </button>
        ))}
      </div>
                ...
                <div key={product.id} className="bg-white border border-primary/10 rounded-2xl overflow-hidden shadow-sm flex flex-col justify-between">
                  <div>
                    ...
                    <div className="p-4">
                      ...
                      <div className="mt-4 flex items-baseline justify-between">
                        ...
                        <span className="text-xs text-gray-700 font-medium bg-gray-100 px-2 py-1 rounded-lg">
                          Qty: {product.quantity}
                        </span>
                      </div>
                      
                      <p className="text-xs text-gray-500 mt-2">
                        Delivery: {product.delivery_time_varies ? 'Varies' : `Usually ${product.delivery_time_manual} days`}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="p-4 pt-0 flex gap-2">
                    <button
                      ...
                      className="flex-1 py-2 bg-gray-50 hover:bg-gray-100 text-gray-700 text-xs font-semibold rounded-xl border border-gray-200 flex items-center justify-center space-x-1 transition"
                    >
                      <Edit3 size={14} />
                      <span>Edit</span>
                    </button>
                    <button
                      ...
                      className="py-2 px-3 bg-red-50 hover:bg-red-100 text-red-600 rounded-xl border border-red-100 flex items-center justify-center transition"
                    >
                      <Trash2 size={14} />
                    </button>
                  </div>
                  ...
                  <div key={order.id} className="bg-white border border-primary/10 rounded-2xl p-4 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        Buyer: <span className="font-medium text-gray-700">{order.buyer_name} ({order.buyer_phone})</span>
                      </p>
                      <p className="text-xs text-gray-500">
                        Quantity: <span className="font-medium text-gray-700">{order.quantity} {order.product_unit}</span> | Total: <span className="font-semibold text-primary">KES {order.total_price}</span>
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {order.status === 'pending' && (
                        <button
                          ...
                          className="w-full md:w-auto flex items-center justify-center space-x-1 py-2 px-4 bg-primary text-white text-xs font-semibold rounded-xl hover:bg-primary-light transition shadow-md shadow-primary/10"
                        >
                          <Check size={14} />
                      ...
                      {order.status === 'delivered' && (
                        <span className="text-xs text-blue-600 font-semibold bg-blue-50 border border-blue-100 py-1.5 px-3 rounded-lg flex items-center space-x-1">
                          <HelpCircle size={14} />
                          <span>Delivered (Awaiting Buyer Confirmation)</span>
                        </span>
                      )}
                      {order.status === 'disputed' && (
                        <span className="text-xs text-red-600 font-semibold bg-red-50 border border-red-100 py-1.5 px-3 rounded-lg flex items-center space-x-1">
                          <AlertCircle size={14} />
                          <span>Disputed (Issue Reported)</span>
                        </span>
                      )}
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-primary/10 overflow-y-auto max-h-[90vh]">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 uppercase tracking-wider">Quantity Available</label>
                  <input
                    type="number"
                    name="quantity"
                    required
